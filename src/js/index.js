...
if (!isIE()) {
    ...
    // Add event listeners to each link with internal reference
    const scrollLinks = document.querySelectorAll('a[href^="#"]');
    for (let i = 0; i < scrollLinks.length; i++) {
        let scrollLink = scrollLinks[i];
...
const openButtons = document.querySelectorAll('[data-openmodal]');
// for (let el in openButtons) {
for (let i = 0; i < openButtons.length; i++) {
    let el = openButtons[i];
...
// Close modals
const closeButtons = document.querySelectorAll('[data-closemodal]');
// for (let el in closeButtons) {
for (let i = 0; i < closeButtons.length; i++) {
    let el = closeButtons[i];
    el.addEventListener('click', function () {
...
function scrollToElement(id) {
    const navbarHeight = document.querySelector('.js-navbar').offsetHeight; // 64
    const scrollTo = document.querySelector(id).offsetTop;
...
function flipScrollButton() {
    ...
    if (scrollButton) {
        if (window.scrollY > vh(45)) { // Magic Number
...
function showOnEnterViewport() {
    ...
    let elementsToShow = document.querySelectorAll('.js-show-on-scroll');
    ...
    for (let i = 0; i < elementsToShow.length; i++) {
        let elem = elementsToShow[i];
        if (isElementInViewport(elem)) {
            elem.classList.add('is-visible');
        } else {
            // to make animations permanent: don't remove visibility
            // elem.classList.remove('is-visible');
        }
    }
}
...
function showNav() {
    ...
    // Show navbar if you're scrolled down a little
...
function toggleNav() {
    ...
    document.querySelector('.js-navbar')
        .classList.toggle('nav-bar--active');

    // Can't be transparent when opened
    let transparent = document.querySelector('.nav-bar--transparent');
    if (transparent) {
...
function iePolyfill() {
    ...
    // Force all 'on-scroll' items visible because IE >:(
    const animated = document.querySelectorAll('.js-show-on-scroll');
    for (let i = 0; i < animated.length; i++) {
        animated[i].classList.add('is-visible');
    }

    // remove carousels because IE
    let carousels = document.querySelectorAll('.carousel.section');
    for (let i = 0; i < carousels.length; i++) {
        carousels[i].parentNode.removeChild(carousels[i]);
    }

    // adjust position of page breadcrumbs cuz dumb IE
    let breadcrumbs = document.querySelector('.banner__content.container');
    if (breadcrumbs) {
